...
spawn(function() 
    ...
    wait(1.5)
    require(game.ReplicatedStorage.Notification).new("<Color=Red>daoquanglam chào ae<Color=/>"):Display()
    wait(.5)
...
local Window = OrionLib:MakeWindow({Name = "daoquanglam", HidePremium = false, IntroText = "daoquanglam⚜️", SaveConfig = true, ConfigFolder = "NightHub"})
...
W:AddLabel("daoquanglam SCRIPT")
W:AddLabel("Discorddaoquanglam")
local Section = W:AddSection({
    Name = "Status"
})
...
pcall(function()
    while wait() do
        if _G.AutoEliteHunter then
            if game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Visible == true then
                OrionLib:MakeNotification({
                    Name = "daoquanglam(Noitification)",
